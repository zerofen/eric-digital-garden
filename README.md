...
奶白底色、青绿色点缀、超大衬线标题和圆形头像。参考用户提供的 Callan 首页截图重新实现，未使用参考站的头像或内容。站内链接使用局部导航，让音乐在栏目切换时保持播放。
...
链接是可选的。音乐文件放在 `public/music/`，并使用 `"audio": "/music/song.mp3"`。添加或更新音频时推荐使用 `npm run music:add` 自动转码并更新歌单，完整步骤见 [音乐更新指南](docs/music-update.md)。
